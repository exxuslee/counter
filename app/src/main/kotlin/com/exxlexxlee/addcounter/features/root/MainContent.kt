...
fun MainContent(
    ...
    CompositionLocalProvider(
        ...
        if (isLandscape) {
            LandscapeLayout(currentRoute, navController, mainViewModel, mainViewState,gameViewModel)
        } else {
            PortraitLayout(currentRoute, navController, mainViewModel, mainViewState, gameViewModel)
        }

        when (viewAction) {
            Action.Dice -> AddCountDialog({
                mainViewModel.clearAction()
            }, onAdd = { count ->
                mainViewModel.clearAction()
                gameViewModel.obtainEvent(Event.AddCount(count))
            })
